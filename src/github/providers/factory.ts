 ...
 */
export function createGitForgeProviderFromEnv(octokits?: Octokits): GitForgeProvider {
  const type = (process.env.FORGE_TYPE || 'github') as 'github' | 'forgejo';
  const apiUrl = process.env.FORGE_API_URL || 
    (type === 'github' ? 'https://api.github.com' : 'https://forgejo.example.com/api/v1');
  const serverUrl = process.env.FORGE_SERVER_URL || 
    (type === 'github' ? 'https://github.com' : 'https://forgejo.example.com');
  const token = process.env.GITHUB_TOKEN || '';

  if (!token) {
    throw new Error("Authentication token is required (GITHUB_TOKEN environment variable)");
  }
 ...
 */
export function isGitHubProvider(provider: GitForgeProvider): provider is GitHubProvider {
 ...
 */
export function isForgejoProvider(provider: GitForgeProvider): provider is ForgejoProvider {
